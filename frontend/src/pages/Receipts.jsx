...
function Receipts() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ supplier: '', products: [{ product: '', quantity: 1 }] });
      setShowForm(false);
      fetchData();
    } catch (err) {
  ...
  const handleValidate = async (id) => {
    try {
      await API.put(`/receipts/validate/${id}`);
      fetchData();
    } catch (err) {
      alert(err.response?.data?.msg || 'Error!');
    }
    ...
    <div className="min-h-screen bg-slate-800 p-8">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h2 className="text-3xl font-bold text-white">Receipts</h2>
            <p className="text-slate-400">Incoming stock from vendors</p>
          </div>
          <button
          ...
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
            <div className="bg-slate-800 rounded-2xl p-8 w-full max-w-lg border border-slate-600 shadow-2xl">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold text-white">New Receipt</h3>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {receipts.map(r => (
              <div key={r._id} className="bg-slate-700 rounded-2xl p-6 border border-slate-600 hover:border-blue-400 transition-all">
                <div className="flex justify-between items-center">
                  <div>
                    <h3 className="text-white font-bold text-lg">{r.supplier}</h3>
                    <p className="text-slate-400 text-sm mt-1">
                      {r.products.map(p => `${p.product?.name || 'N/A'} x${p.quantity}`).join(', ')}
                    </p>
                    <p className="text-slate-500 text-xs mt-1">{new Date(r.createdAt).toLocaleDateString()}</p>
                  </div>
            ...
            {receipts.length === 0 && (
              <div className="text-center text-slate-400 py-20">No receipts yet!</div>
            )}
          </div>
        )}
      </div>
